...
class FashionMNIST(Dataset):  # [DS-FM]
    data_root = 'dataset/fashion_mnist'
    ...
    def __init__(self, train=True, count=10000, specific=None, out_class_scale=1, per_out_class_count=None, noise_std=0.155*2, verbose=False):
        ...
        if train:
            dataset = FashionMNIST.train_dataset
            ...
            dataset = FashionMNIST.test_dataset
        ...
        if verbose:
            print(f"Loaded {len(self.data)} Fashion-MNIST images")
...
def train(specific, checkpoint_dir, epoch=40, step=5, find_best=False):
    ...
    batch_size = 128
    train_count = (FashionMNIST.train_dataset.targets == specific).sum().item() // batch_size * batch_size
    data_dataset = FashionMNIST(specific=specific, count=train_count)
    ...
    # 选取最好的Epoch
    print(f"ALOCC: ", end='')
    test_dataset = FashionMNIST(train=False, specific=specific, count=1600, out_class_scale=0.5)
...
def train_loss(specific, checkpoint_dir, epoch=40, step=5, oc_cnt=600, find_best=False):
    ...
    batch_size = 128
    train_count = (FashionMNIST.train_dataset.targets == specific).sum().item() // batch_size * batch_size
    data_dataset = FashionMNIST(train=True, specific=specific, count=train_count)
    data_loader = DataLoader(data_dataset, batch_size=batch_size, shuffle=True)
    model = ALOCC_LOSS(in_h=28, out_h=28)
    # print(outclass_files)
    oc_cnt = max(batch_size, (oc_cnt // (9*batch_size)) * batch_size)
    outclass_dataset = FashionMNIST(train=True, count=0, specific=specific, per_out_class_count=oc_cnt)
...
def test_single_class(specific_number):
    data_dataset = FashionMNIST(train=False, count=400, specific=specific_number, out_class_scale=1)
...
def test_per_class(specific_number):
    data_dataset = FashionMNIST(train=False, count=800, specific=specific_number, per_out_class_count=200)
    data_loader = DataLoader(data_dataset, batch_size=256, shuffle=False)
    model = ALOCC(c_dim=1, gf_dim=16, df_dim=16, in_h=28, out_h=28)
    model._load_checkpoint(f'./checkpoint/mnist/{specific_number}/best.pth')
    f1, acc, eer, auc, ssim_ic, ssim_oc, vif_ic, vif_oc = calculate_metrics(model, data_loader=data_loader, inner_class=specific_number)
