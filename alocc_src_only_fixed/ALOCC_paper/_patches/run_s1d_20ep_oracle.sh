#!/bin/bash
# [S1D 20ep-Oracle] 50-run matrix
# S1 bottleneck + L1-hinge distortion + D outclass
#
# Usage:
#   bash run_s1d_20ep_oracle.sh --smoke    # digit=1 seed=42 only (~4 min)
#   bash run_s1d_20ep_oracle.sh           # full 50 runs (~44 min)
...
echo "[S1D-20EP-ORACLE] total=$TOTAL runs"
...
for C in "${DIGITS[@]}"; do
    ...
    for SEED in "${SEEDS[@]}"; do
        CELL_IDX=$((CELL_IDX + 1))
        OUT_DIR="$OUTPUT_ROOT/s1d_20ep_c${C}_seed${SEED}_oracle"
        LOG_FILE="$LOGS_DIR/c${C}_s${SEED}.log"
...
echo "[S1D-20EP-ORACLE] done. elapsed=${TOTAL_DUR}min failed=${#FAILED[@]}"
if [ ${#FAILED[@]} -gt 0 ]; then
    echo "FAILED: ${FAILED[*]}"
    exit 1
fi
